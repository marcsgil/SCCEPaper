...
function energy(θ::Number,par,fy,fx,H)
    prob = IntegralProblem((X,par)->integrand(X,θ,par,fy,fx,(sol,i)->energy_output(sol,i,θ,par,X,H)),fill(-Inf,2),fill(Inf,2),par,batch=16,nout=2)
    sol = solve(prob,CubatureJLh(),reltol=1e-3,abstol=1e-4)
    sol[2]/sol[1]
end

function energy(θs::AbstractArray,par,fy,fx,H)
    map(θ->energy(θ::Number,par,fy,fx,H),θs)
end
...
function heat(θs::AbstractArray,par,fy,fx,H,H2)
    map(θ->heat(θ::Number,par,fy,fx,H,H2),θs)
end